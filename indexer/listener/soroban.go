...
func (l *EventListener) Start(ctx context.Context) error {
	...
	if currentLedger > 0 { slog.Info("Resuming event indexing from checkpoint", "startLedger", currentLedger) } else { startLedger, err := db.GetLatestProcessedLedger(ctx); if err != nil { return fmt.Errorf("failed to get latest processed ledger: %w", err) }; if startLedger > 0 { currentLedger = startLedger + 1 } else { latest, err := l.getLatestLedgerSequence(ctx); if err != nil { return fmt.Errorf("failed to get latest ledger sequence: %w", err) }; currentLedger = latest } }
	pollInterval := time.Duration(l.cfg.IndexerPollIntervalMs) * time.Millisecond; if pollInterval <= 0 { pollInterval = 5 * time.Second }; ticker := time.NewTicker(pollInterval); defer ticker.Stop()
...
func (l *EventListener) pollEvents(ctx context.Context, startLedger int32) (int32, error) {
	...
	filters := []EventFilter{{Type: "contract", ContractIDs: contractIDs}}; cursor := ""; var latestLedgerSeq int32; var events []SorobanEvent
	for { params := GetEventsParams{StartLedger: startLedger, Filters: filters, Pagination: &PaginationParams{Limit: 100, Cursor: cursor}}; var res GetEventsResult; if err := api.CallSorobanRPC(ctx, l.cfg.SorobanRPCURL, "getEvents", params, &res); err != nil { return startLedger, fmt.Errorf("call getEvents (startLedger=%d, cursor=%s): %w", startLedger, cursor, err) }; if res.LatestLedger != 0 { latestLedgerSeq = int32(res.LatestLedger) }; for _, ev := range res.Events { events = append(events, SorobanEvent{ID: ev.ID, ContractID: ev.ContractID, Ledger: ev.Ledger, LedgerClosedAt: ev.LedgerClosedAt, Topic: ev.Topic, Value: ev.Value.Xdr}) }; if res.Cursor != "" && len(res.Events) > 0 { cursor = res.Cursor } else { break } }
	ids := make([]string, 0, len(events)); for _, event := range events { ids = append(ids, event.ID) }; processed, err := db.AreEventsProcessed(ctx, ids); if err != nil { slog.Error("Failed to check if events are processed", "error", err); processed = nil }
